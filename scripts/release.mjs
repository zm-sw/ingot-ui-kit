...
import { writeReleases, currentPages } from "./build-releases.mjs";
...
/**
 * The doc web's list of releases, written in the SAME commit as the
 * version.
 *
 * It used to be regenerated by hand, which means it was regenerated once:
 * the site shipped v1.2.0 while its own Changes page ended at v1.1.1, the
 * pin example advised a stale tag, and the newest eighteen primitives had
 * no "since" badge. The tag for this release does not exist yet — it is
 * cut after the merge — so it is handed in rather than read out of git.
 */
writeReleases({
  pending: {
    tag: `v${next}`,
...
git("add", pkgPath, "CHANGELOG.md", "src/ingot-docs/releases.json");
