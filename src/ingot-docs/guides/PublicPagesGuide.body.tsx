...
const body: IngotGuideBody = {
  ...
  "hlavicka-sekce": {
    cs: (
      <div className="space-y-3 text-sm text-ink-2">
        <p>
          Dvousloupec: štítek s nadpisem vlevo, uvozující odstavec vpravo. Akcent nese
          jen štítek. Pod hlavičkou trojice featur — ikona 18 px v akcentovém rámečku,
          titulek a malý text. Trojice stojí v jednom zaobleném rámu a panely dělí
          vlasová linka, ne mezera: jsou to tři části jednoho celku, ne tři kartičky.
        </p>
    ...
    en: (
      <div className="space-y-3 text-sm text-ink-2">
        <p>
          A two-column head: the label and heading on the left, the lede on the right.
          Only the label carries the accent. Below it a trio of features — an 18 px icon
          in an accent frame, a title and small text. The trio sits in one rounded frame
          with the panels split by a hairline rather than a gap: three parts of one
          whole, not three small cards.
        </p>
  ...
  kroky: {
    cs: (
      <div className="space-y-3 text-sm text-ink-2">
        <p>
          Karty s pořadovým číslem a šipkou k dalšímu kroku; poslední karta šipku nemá.
          Číslo je malá akcentová pilulka, ne velké číslo — nejvyšší váhu v kartě má
          titulek kroku. Číslo se počítá z pořadí, takže se nedá přeskočit.
    ...
    en: (
      <div className="space-y-3 text-sm text-ink-2">
        <p>
          Cards with an ordinal number and an arrow towards the next step; the last card
          has no arrow. The number is a small accent pill, not a large numeral — the
          step's title carries the most weight in the card. The number is derived from
          the order, so it cannot skip. The three-column grid is the default; four
          columns belong to process steps only.
        </p>
  ...
  segmenty: {
    cs: (
      <div className="space-y-3 text-sm text-ink-2">
        <p>
          Karty „pro koho to je" se štítky. Štítek je hranatý mono malými písmeny — je
          to technický údaj o provozu, ne stav entity. Štítky zůstávají neutrální;
    ...
    en: (
      <div className="space-y-3 text-sm text-ink-2">
        <p>
          "Who is it for" cards with tags. A tag is a square lowercase mono label — a
          technical fact about a shop, not the status of an entity. Tags stay neutral;
  ...
  srovnani: {
    cs: (
      <div className="space-y-3 text-sm text-ink-2">
        <p>
          Řádková tabulka úkol / dnes / s platformou. Řádek je ta podstatná jednotka:
          páruje jeden úkol s oběma stavy, takže se nedá napsat rozpojeně. Hlavička sedí
          na tlumené ploše, třetí sloupec je zvýrazněný a je jediným akcentovým prvkem
          sekce. Na úzké obrazovce se tabulka roluje do strany — složená mřížka už nic
          nesrovnává.
        </p>
    ...
    en: (
      <div className="space-y-3 text-sm text-ink-2">
        <p>
          ...
          matters: it pairs one task with both states, so it cannot be written apart.
          The header sits on the muted surface, the third column is highlighted and is
          the section's only accented element. On a narrow screen the table scrolls
  ...
  cenik: {
    cs: (
      <div className="space-y-3 text-sm text-ink-2">
        <p>
          ...
          hlavičce karty vedle názvu plánu. Cena je mono, akce má každá karta a všechny
          sedí na jedné patě. Blok neumí vykreslit nic, co nedostane přes{" "}
          <IngotCode>plans</IngotCode> — názvy, ceny i výčty vlastností jsou data plánů
          platformy, žádné částky v kódu. Ceny v ukázce jsou proto schválně vyiksované.
        </p>
    ...
    en: (
      <div className="space-y-3 text-sm text-ink-2">
        <p>
          Three plan cards, the middle one highlighted with an outline and a shadow; the
          badge sits in the card header next to the plan name. The price is set in mono,
          every card carries an action and all of them rest on one foot line. The block
          cannot render anything it does not receive through{" "}
          <IngotCode>plans</IngotCode> — names, prices and feature lists are the
          platform's plan data, no amounts in code. That is why the example prices are
          X-ed out on purpose.
        </p>
  ...
  faq: {
    cs: (
      <div className="space-y-3 text-sm text-ink-2">
        <p>
          Otázka je tlačítko s ohlášeným stavem rozbalení, odpověď je pojmenovaná oblast
          — celé ovladatelné klávesnicí. Odpověď je povinné pole: položku bez odpovědi
          nejde napsat.
        </p>
        <FaqExample lang="cs" />
      </div>
    ...
    en: (
      <div className="space-y-3 text-sm text-ink-2">
        <p>
          ...
          named region — all keyboard-operable. The answer is a required field: an item
  ...
  cta: {
    cs: (
      <div className="space-y-3 text-sm text-ink-2">
        <p>
          ...
          ploše kam jít pohled. Kreslí se prohozenými neutrálními tokeny, takže v tmavém
          motivu se obrátí sám. Na stránce smí být tmavý blok nejvýš dvakrát — tohle CTA
          a patička.
        </p>
        <CtaExample lang="cs" />
      </div>
    ...
    en: (
      <div className="space-y-3 text-sm text-ink-2">
        <p>
          A dark block with two large actions; the primary one is accented so the eye
          has somewhere to land on the dark surface. It is drawn with the neutral tokens
          swapped, so it inverts itself in the dark theme. A page may hold at most two
          dark blocks — this call to action and the footer.
        </p>
        <CtaExample lang="en" />
      </div>
    ),
