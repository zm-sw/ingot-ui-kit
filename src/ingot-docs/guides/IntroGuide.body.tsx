...
const body: IngotGuideBody = {
  ...
  "jak-se-pouziva": {
    ...
    en: (
      <div className="space-y-3 text-sm text-ink-2">
        ...
        <p>
          <strong>Always through this one entry point</strong>, never by a deeper path
          to a specific file. What comes out of it is the kit’s public interface and
          changes are announced; what does not is internal and may be renamed or split
          at any time. That interface includes the types an element asks for —{" "}
  ...
  "jak-pridat": {
    cs: (
      <div className="space-y-3 text-sm text-ink-2">
        <p>
          Nové primitivum vzniká, <strong>až si o něj řekne konkrétní obrazovka</strong>
          . Když ten žadatel je, patří k sobě tři věci v jedné změně: komponenta a její
          vyvedení z <IngotCode>@/ingot</IngotCode>, stránka na tomhle webu s živou
          ukázkou, a první obrazovka, která ji doopravdy používá.
        </p>
        <p>
          Ukázku musí stránka vykreslit <strong>skutečnou komponentou</strong>. Opsané
          JSX vypadá v den zápisu stejně a od druhého dne tiše lže.
        </p>
      </div>
    ),
    en: (
      <div className="space-y-3 text-sm text-ink-2">
        <p>
          A new primitive appears <strong>once a concrete screen asks for it</strong>.
          Once there is such a caller, three things belong together in one change: the
          component and its export from <IngotCode>@/ingot</IngotCode>, a page on this
          site with a live demo, and the first screen that genuinely uses it.
        </p>
        <p>
          The page must render the demo with the <strong>real component</strong>. Copied
          JSX looks identical on the day it is written and lies quietly from the second
          day on.
        </p>
      </div>
    ),
  ...
  "co-tu-najdes": {
    cs: (
      <div className="space-y-3 text-sm text-ink-2">
        <p>
          Každá stránka komponenty má stejnou stavbu: živou ukázku i s kódem, kdy použít
          a kdy nepoužít, tabulku vlastností, přístupnost a překlady. Tam, kde první
          verze něco schválně neumí, je to vypsané taky. Pravidlo o překladech je
          společné všem prvkům, takže má vlastní stránku —{" "}
    ...
    en: (
      <div className="space-y-3 text-sm text-ink-2">
        <p>
          ...
          accessibility and translations. Where the first version deliberately cannot do
          something, that is listed too. The rule about translations is common to every
          element, so it has a page of its own —{" "}
          <a className="underline" href="#/preklady">
            Translations
          </a>
          .
