...
export const IngotRowActionsDoc: IngotDocPage = {
  ...
  avoidWhen: {
    cs: [
      <>
        ...
        <IngotCode>IngotButton</IngotCode> s popiskem — tenhle rozpočet platí jen v
        řádku, kde se opakuje dvacetkrát.
      </>,
      <>
        Čtyři a víc akcí. Řada ikon bez popisků se přestane číst a jediné rozlišení je
        tvar piktogramu; přesuň je do menu.
      </>,
      <>
        Samotné mazání. Nevratná akce má tón <IngotCode>danger</IngotCode>, ale sama
        nemaže — potvrzení řeší <IngotCode>IngotConfirm</IngotCode> u volajícího. Tón
        mění jen to, jak tlačítko vypadá při najetí.
      </>,
    ],
    en: [
      <>
        The primary action of a screen. “New order” in a header is an{" "}
        <IngotCode>IngotButton</IngotCode> with a label — this budget only applies in a
        row, where it repeats twenty times.
      </>,
      <>
        Four or more actions. A row of unlabelled icons stops being read and the only
        distinction left is the shape of the glyph; move them into a menu.
      </>,
      <>
        Deleting on its own. An irreversible action carries the{" "}
        <IngotCode>danger</IngotCode> tone, but it does not delete by itself —
        confirmation is handled by <IngotCode>IngotConfirm</IngotCode> at the caller.
        The tone only changes how the button looks on hover.
      </>,
    ],
  },
  props: [
  ...
  a11y: {
    cs: [
      <>
        Popisek je <strong>povinný a začíná slovesem</strong>. Bez něj odečítač přečte
        jen „tlačítko“, dvacetkrát pod sebou — proto „Smazat objednávku“, ne „Koš“.
      </>,
      <>
        Týž popisek jde do <IngotCode>aria-label</IngotCode> i do{" "}
        <IngotCode>title</IngotCode>, takže se stejná věta ukáže v bublině i ohlásí
        odečítači. Jedna pravda, dvě cesty k ní.
      </>,
      <>
        Pevné pořadí je přístupnost, ne estetika. Přeházené pořadí je nejrychlejší cesta
        ke smazání špatného řádku — pro myš i pro klávesnici.
      </>,
      <>
        Zakázané tlačítko zůstává v pořadí i ve výpisu odečítače, jen ztlumené a
        nepoužitelné; kurzor nad ním hlásí, že akci tenhle řádek nedovoluje.
      </>,
    ],
    en: [
      <>
        The label is <strong>required and starts with a verb</strong>. Without it a
        screen reader announces just “button”, twenty times in a row — so “Delete
        order”, not “Trash”.
      </>,
      <>
        The same label goes into <IngotCode>aria-label</IngotCode> and{" "}
        <IngotCode>title</IngotCode>, so the same sentence appears in the tooltip and is
        announced to a screen reader. One truth, two routes to it.
      </>,
      <>
        The fixed order is accessibility, not aesthetics. A reshuffled order is the
        fastest route to deleting the wrong row — for the mouse and for the keyboard
        alike.
      </>,
      <>
        A disabled button keeps its place in the order and in the screen reader's
        listing, only dimmed and inert; the cursor over it says this row does not allow
        the action.
      </>,
    ],
  },
  i18n: {
    cs: [
      <>
        Popisky dodává volající už přeložené — kit vlastní jmenný prostor překladů nemá.
      </>,
      <>
        Popisek začíná slovesem v každém jazyce. Překlad, který sloveso vypustí, ruší to
        jediné, co odečítači o akci něco říká.
      </>,
      <>
        Na délce popisku nezáleží — je jen v bublině a v odečítači, ne v rozvržení.
        Tlačítko má 28×28 px bez ohledu na jazyk, takže se vyplatí popsat akci celou
        větou.
      </>,
    ],
    en: [
      <>
        Labels are supplied by the caller already translated — the kit has no
        translation namespace of its own.
      </>,
      <>
        The label starts with a verb in every language. A translation that drops the
        verb removes the only thing that tells a screen reader what the action does.
      </>,
      <>
        The label's length does not matter — it lives in the tooltip and in the screen
        reader, not in the layout. The button is 28×28 px in any language, so it pays to
        describe the action in a full phrase.
      </>,
    ],
  },
};
