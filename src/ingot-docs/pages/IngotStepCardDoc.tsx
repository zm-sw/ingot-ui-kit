...
export const IngotStepCardDoc: IngotDocPage = {
  ...
  useWhen: {
    cs: [
      <>
        Nastavení, které se nedělá jedním dlouhým formulářem, ale kroky.
        Jednotlivé kroky se dokončují v různé dny a různými lidmi, takže
        každý z nich potřebuje vlastní rám a vlastní stav.
      </>,
      <>
        Krok, jehož obsahem je seznam nebo vnořená tabulka toho, co v něm
        vzniklo — země a měny, skupiny vlastností. Do těla karty patří
        právě tenhle výčet, ne pole formuláře jedno pod druhým.
      </>,
      <>
        Přehled, ve kterém má být na první pohled vidět, co už je hotové.
        Hotový krok má zelené záhlaví a fajfku místo čísla, takže se stav
        čte z odstupu.
      </>,
      <>
    ...
    en: [
      <>
        A setup that is not done through one long form but in steps.
        Individual steps get finished on different days by different
        people, so each one needs its own frame and its own state.
      </>,
      <>
        A step whose content is a list or a nested table of what it
        produced — countries and currencies, attribute groups. That
        listing is what belongs in the card body, not form fields stacked
        one below the other.
      </>,
      <>
        An overview where it must be obvious at a glance what is already
        done. A finished step has a green header and a check mark instead
        of the number, so the state reads from a distance.
      </>,
      <>
  ...
  avoidWhen: {
    cs: [
      <>
        Krok, který se potvrzuje tlačítkem. Krok je hotový tehdy, když má,
        co potřebuje — tlačítko „Hotovo“ v patičce by zavedlo druhý stav,
        který s obsahem karty nesouvisí.
      </>,
      <>
        Průvodce, který ukazuje jen jednu obrazovku po druhé a předchozí
        krok zahodí. Tahle karta stojí na tom, že všechny kroky zůstávají
        vidět vedle sebe.
      </>,
      <>
        Obyčejné seskupení obsahu bez pořadí a bez stavu. Očíslované
        záhlaví a fajfka tam slibují postup, který ve skutečnosti neexistuje.
      </>,
      <>
        Zvýraznění chyby nebo varování. Karta zná jediný stav — hotovo,
        nebo ne; červené a oranžové hlášky patří dovnitř obsahu.
      </>,
    ],
    en: [
      <>
        A step that gets confirmed with a button. A step is done when it
        has what it needs — a “Done” button in the footer would introduce a
        second state unrelated to the card's content.
      </>,
      <>
        A wizard that shows one screen at a time and discards the previous
        step. This card is built on all the steps staying visible side by
        side.
      </>,
      <>
        Plain grouping of content with no ordering and no state. A numbered
        header and a check mark promise progress that does not actually
        exist there.
      </>,
      <>
        Highlighting an error or a warning. The card knows a single state —
        done or not; red and amber messages belong inside the content.
      </>,
    ],
  },
  props: [
  ...
  a11y: {
    cs: [
      <>
        Hotový krok je poznat <strong>tvarem</strong>, ne jen barvou: zelené
        záhlaví doprovází fajfka místo čísla. Kdo barvu nerozliší, čte tvar
        — a tenhle stav je jediná informace, kvůli které se člověk na
        obrazovku vrací.
      </>,
      <>
        Fajfka nese popisek z <IngotCode>doneLabel</IngotCode>. Vynechat ho
        znamená, že odečítač ohlásí kartu bez toho jediného, co ji odlišuje
        od rozdělané.
      </>,
      <>
        Kicker je mono verzálkami jen opticky — text předávej normálně
        psaný. Verzálky napsané ručně čte odečítač po písmenech.
      </>,
      <>
        Patička obsahuje jednu akci. Víc tlačítek v ní udělá z jednoznačného
    ...
    en: [
      <>
        A finished step is recognizable by <strong>shape</strong>, not color
        alone: the green header comes with a check mark instead of the
        number. Anyone who cannot tell the color apart reads the shape — and
        this state is the one piece of information people come back to the
        screen for.
      </>,
      <>
        The check mark carries the label from <IngotCode>doneLabel</IngotCode>.
        Leaving it out means a screen reader announces the card without the
        single thing that separates it from an unfinished one.
      </>,
      <>
        The kicker is mono uppercase only visually — pass the text written
        normally. Uppercase typed by hand gets read out letter by letter.
      </>,
      <>
        The footer holds one action. More buttons turn an unambiguous “add
  ...
  i18n: {
    cs: [
      <>
        Všechny popisky dodává volající přeložené — kit vlastní jmenný
        prostor překladů nemá. Týká se to i{" "}
        <IngotCode>doneLabel</IngotCode>, které se snadno zapomene, protože
        není vidět.
      </>,
      <>
        Číslo v <IngotCode>step</IngotCode> se nepřekládá, ale slovo
        v <IngotCode>kicker</IngotCode> ano. Drž ho na jednom slově a čísle
        — záhlaví má na kicker jeden řádek.
      </>,
      <>
    ...
    en: [
      <>
        All labels are passed in already translated — the kit has no
        translation namespace of its own. That includes{" "}
        <IngotCode>doneLabel</IngotCode>, which is easy to forget because it
        is not visible.
      </>,
      <>
        The number in <IngotCode>step</IngotCode> is not translated, but the
        word in <IngotCode>kicker</IngotCode> is. Keep it to one word and a
        number — the header gives the kicker a single line.
      </>,
      <>
